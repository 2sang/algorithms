...
int** floyd_warshall(int vertex_count, int** l, int** w, int** pred){
...
int main(){
	...
	int **weight_matrix = load_inputs(&vertex_count, &edge_count);
	int **pred = new_INF_matrix(vertex_count);
	...
	printf("INITIAL_ADJACENT_MATRIX :\n");
	print_matrix(vertex_count, adj_matrix);

	for(int i = 0; i < vertex_count - 1; i++){
		printf("EXTEND_SHORTEST_PATH #%d\n", i+1);
		adj_matrix = floyd_warshall(vertex_count, adj_matrix, weight_matrix, pred);
		print_matrix(vertex_count, adj_matrix);
		printf("PREDECESSOR_MATRIX #%d\n", i+1);
		print_matrix(vertex_count, pred);
		printf("\n");
	}
